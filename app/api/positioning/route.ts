...
import { POSITIONING_SYSTEM_PROMPT, buildPositioningPrompt } from '@/lib/blueprintPrompt';
import { positioningOutputSchema, positioningJsonSchema } from '@/lib/admissions/positioningSchema';
import type { PositioningHypothesis, PositioningState } from '@/lib/admissions/journey';
...
/** Forced tool call + zod validation + one retry. `strict:false` skips grammar
 *  compilation (the hypotheses array is too large for the strict grammar). */
async function callStructured<T>(
  client: Anthropic,
  opts: {
    system: string; prompt: string; toolName: string; description: string;
    inputSchema: Record<string, unknown>; zodSchema: z.ZodType<T>; maxTokens: number; strict?: boolean;
  },
): Promise<T> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (message.stop_reason === 'max_tokens') { feedback = 'Output was truncated (max_tokens). Be more concise.'; continue; }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: 'Invalid positioning request' }, { status: 400 });
    ...
    if (studentError) { console.error('positioning: student load failed:', studentError); return NextResponse.json({ error: 'Could not load the student.' }, { status: 500 }); }
    if (!studentRow) return NextResponse.json({ error: 'Student not found' }, { status: 404 });
    const student = studentRow.data as Student;

    // Reuse the assessment from an existing strategy when present.
      ...
      async start(controller) {
        ...
        try {
          ...
          const out = await callStructured(client, {
            system: POSITIONING_SYSTEM_PROMPT, prompt: buildPositioningPrompt(student, assessment),
            toolName: 'submit_positioning', description: 'Submit 3–5 evidence-backed positioning hypotheses for the student to validate.',
            inputSchema: positioningJsonSchema(), zodSchema: positioningOutputSchema, maxTokens: 8000,
            strict: false,
          });

          const hypotheses: PositioningHypothesis[] = out.hypotheses.map((h, i) => ({ ...h, id: `h${i + 1}` }));
          const state: Pick<PositioningState, 'generatedAt' | 'hypotheses'> = {
          ...
          controller.enqueue(encoder.encode(JSON.stringify({ error: err instanceof Error ? err.message : 'Positioning generation failed' })));
        } finally {
    ...
    console.error('Positioning route error:', err);
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Positioning generation failed' }, { status: 500 });
  }
}
